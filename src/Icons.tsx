...
export const I = {
  ...
  Kbd: ({ children }: { children: ReactNode }) => <span style={{ background: '#2d2d2d', border: '1px solid #444', borderRadius: 3, padding: '0 5px', fontSize: 12, color: '#999', fontFamily: 'monospace', lineHeight: '18px', display: 'inline-block' } as CSSProperties}>{children}</span>,
...
export const Av = ({ name, size = 28 }: { name: string; size?: number }) => {
  ...
  return (
    <div style={{
      ...
      background: isU ? 'transparent' : c + '22',
      border: isU ? `1.5px dashed ${c}` : `1.5px solid ${c}`,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: size * .36, fontWeight: 600, color: c, flexShrink: 0,
      letterSpacing: '-.02em'
    }}>
