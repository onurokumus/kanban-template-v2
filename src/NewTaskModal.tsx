...
export const NewTaskModal = ({ onClose, onAdd, defaultStatus, toast: addToast }: Props) => {
  ...
  const is: CSSProperties = { background: 'var(--hover)', color: 'var(--text-dim)', border: '1px solid var(--border)', borderRadius: 4, padding: '6px 10px', fontSize: 13, outline: 'none', fontFamily: 'inherit', width: '100%', boxSizing: 'border-box', transition: 'border-color .2s, box-shadow .2s' };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 5000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      ...
      `}</style>
      <div ref={ref} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, width: 480, boxShadow: '0 20px 60px rgba(0,0,0,.5)', animation: 'slideIn .2s ease-out' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ color: 'var(--text-main)', fontSize: 16, fontWeight: 600 }}>New Task</span>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--text-subtle)', cursor: 'pointer', padding: 4 }}><I.X /></button>
        </div>
        <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div><label style={{ color: 'var(--text-subtle)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', marginBottom: 4, display: 'block' }}>Title *</label><input value={f.title} onChange={e => sF({ ...f, title: e.target.value })} style={is} autoFocus placeholder="What needs to be done?" /></div>
          <div><label style={{ color: 'var(--text-subtle)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', marginBottom: 4, display: 'block' }}>Description</label><textarea value={f.desc} onChange={e => sF({ ...f, desc: e.target.value })} style={{ ...is, minHeight: 60, resize: 'vertical' }} placeholder="Add details..." /></div>
          ...
          <div><label style={{ color: 'var(--text-subtle)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Tags</label><div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>{TAGS.map(t => <button key={t} onClick={() => sF({ ...f, tags: f.tags.includes(t) ? f.tags.filter(x => x !== t) : [...f.tags, t] })} style={{ background: f.tags.includes(t) ? TC[t] : 'var(--bg-alt)', color: f.tags.includes(t) ? '#ddd' : 'var(--text-subtle)', border: '1px solid ' + (f.tags.includes(t) ? 'transparent' : 'var(--border)'), fontSize: 11, padding: '3px 10px', borderRadius: 12, cursor: 'pointer', transition: 'all .15s' }}>{t}</button>)}</div></div>
          <button onClick={submit} style={{ background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 4, padding: '10px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginTop: 6, transition: 'background .2s' }}>Create Task</button>
        </div>
      </div>
    </div>
  );
