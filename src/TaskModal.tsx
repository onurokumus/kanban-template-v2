...
import { MEMBERS, PRIORITIES, TAGS, MC, PC, TC, coid, sid, fmt, today, addD } from "./constants.ts";
...
export const TaskModal = ({ task, onClose, onUpdate, onDelete, allTasks, toast }: Props) => {
  ...
  const ips: CSSProperties = { background: '#2d2d2d', color: '#ccc', border: '1px solid #3c3c3c', borderRadius: 4, padding: '4px 8px', fontSize: 13, outline: 'none', fontFamily: 'inherit', transition: 'all .2s' };
  ...
  const fieldR = (label: string, field: string, value: string | number, type = 'text') => (
    <div style={{ display: 'flex', alignItems: 'flex-start', padding: '7px 0', borderBottom: '1px solid #2d2d2d', gap: 10 }}>
      <span style={{ width: 105, flexShrink: 0, color: '#888', fontSize: 13, paddingTop: 3 }}>{label}</span>
      {ef === field ? (
        <div style={{ display: 'flex', gap: 6, flex: 1, alignItems: 'center' }}>
          ...
          <button onClick={() => saveE(field)} style={{ background: '#007acc', color: '#fff', border: 'none', borderRadius: 3, padding: '4px 12px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Save</button>
          <button onClick={() => setEf(null)} style={{ background: '#3c3c3c', color: '#aaa', border: 'none', borderRadius: 3, padding: '4px 10px', fontSize: 13, cursor: 'pointer' }}>✕</button>
        </div>
      ) : (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', minHeight: 20 }} onClick={() => startE(field, value)}>
          {field === 'priority' ? <span style={{ color: PC[value as string], fontSize: 14, fontWeight: 600 }}>{value}</span>
            : field === 'assignee' ? <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Av name={value as string} size={20} /><span style={{ color: '#ccc', fontSize: 14 }}>{value}</span></div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 5000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      ...
      `}</style>
      <div ref={ref} style={{ background: '#1e1e1e', border: '1px solid #3c3c3c', borderRadius: 8, width: 680, maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,.5)', animation: 'slideIn .2s ease-out' }}>
        {/* Header */}
        <div style={{ padding: '14px 20px', borderBottom: '1px solid #2d2d2d', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ color: PC[task.priority], fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', background: PC[task.priority] + '18', padding: '3px 8px', borderRadius: 3 }}>{task.priority}</span>
          <span style={{ color: '#d4d4d4', fontSize: 16, fontWeight: 600, flex: 1 }}>{task.title}</span>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #2d2d2d', padding: '0 20px' }}>
          {(["details", "subtasks", "dependencies", "comments", "activity"] as TabKey[]).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: '9px 14px', background: 'none', border: 'none', borderBottom: tab === t ? '2px solid #007acc' : '2px solid transparent', color: tab === t ? '#d4d4d4' : '#888', fontSize: 13, fontWeight: 500, cursor: 'pointer', textTransform: 'capitalize', marginBottom: -1 }}>
        ...
        <div style={{ padding: '14px 20px', overflowY: 'auto', flex: 1 }}>
          {tab === 'details' && <div>
            ...
            <div style={{ padding: '7px 0', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              <span style={{ width: 105, flexShrink: 0, color: '#888', fontSize: 13 }}>Tags</span>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>{TAGS.map(t => (
                <button key={t} onClick={() => { const c = task.tags || []; onUpdate(task.id, { tags: c.includes(t) ? c.filter(x => x !== t) : [...c, t] }); }} style={{ background: (task.tags || []).includes(t) ? TC[t] : '#2d2d2d', color: (task.tags || []).includes(t) ? '#ddd' : '#666', border: '1px solid ' + ((task.tags || []).includes(t) ? 'transparent' : '#3c3c3c'), fontSize: 12, padding: '2px 8px', borderRadius: 3, cursor: 'pointer' }}>{t}</button>
              ))}</div>
            </div>
            <div style={{ padding: '7px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 105, flexShrink: 0, color: '#888', fontSize: 13 }}>Progress</span>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12 }}>
                <input 
                  ...
                  style={{ 
                    flex: 1, 
                    accentColor: MC[task.assignee] || '#007acc', 
                    cursor: 'pointer',
                    height: 6
                  }} 
                />
                <span style={{ fontSize: 13, color: MC[task.assignee] || '#007acc', fontWeight: 600, width: 35, textAlign: 'right' }}>{task.progress || 0}%</span>
              </div>
            </div>
            <div style={{ marginTop: 8, fontSize: 12, color: '#555', display: 'flex', gap: 12 }}>
          ...
          {tab === 'subtasks' && <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 14, color: '#999' }}>Track progress with subtasks</span>
              <button onClick={addSub} style={{ background: '#007acc', color: '#fff', border: 'none', borderRadius: 3, padding: '4px 12px', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}><I.Plus />Add</button>
            </div>
              ...
              return (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: '1px solid #2d2d2d' }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <span style={{ fontSize: 14, color: s.done ? '#666' : '#ccc', textDecoration: s.done ? 'line-through' : 'none' }}>{s.title}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <DatePicker value={s.ganttStart || ''} onChange={v => upDate('ganttStart', v)} style={{ maxWidth: 120 }} />
                      <span style={{ fontSize: 11, color: '#444' }}>→</span>
                      <DatePicker value={s.deadline || ''} onChange={v => upDate('deadline', v)} style={{ maxWidth: 120 }} />
                    </div>
                  </div>
            ...
            {(task.subtasks || []).length === 0 && <p style={{ color: '#444', fontSize: 14, textAlign: 'center', padding: 20 }}>No subtasks. Click "Add" to break this task down.</p>}
          </div>}

          {tab === 'dependencies' && <div>
            <p style={{ fontSize: 14, color: '#999', marginBottom: 12 }}>Select tasks that must be completed before this one:</p>
            {allTasks.filter(t => t.id !== task.id && t.status !== 'completed').map(t => (
              <div key={t.id} onClick={() => toggleDep(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 4, cursor: 'pointer', marginBottom: 4, background: (task.dependencies || []).includes(t.id) ? '#007acc15' : '#252526', border: `1px solid ${(task.dependencies || []).includes(t.id) ? '#007acc44' : '#2d2d2d'}` }}>
                ...
                <span style={{ fontSize: 14, color: '#ccc', flex: 1 }}>{t.title}</span>
                <span style={{ fontSize: 11, color: PC[t.priority], fontWeight: 600 }}>{t.priority}</span>
              </div>
            ))}
          </div>}
          ...
          {tab === 'comments' && <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, position: 'relative' }}>
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                <div style={{ display: 'flex', gap: 6 }}>
                  <input ref={inputRef} value={ct} onChange={e => handleCommentInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addComment(); }} placeholder="Add a comment... (@ to mention)" style={{ flex: 1, background: '#2d2d2d', color: '#ccc', border: '1px solid #3c3c3c', borderRadius: 4, padding: '7px 10px', fontSize: 14, outline: 'none', fontFamily: 'inherit' }} />
                ...
                {showMentions && <div style={{ position: 'absolute', top: '100%', left: 0, background: '#252526', border: '1px solid #3c3c3c', borderRadius: 4, padding: 4, zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,.4)', marginTop: 4, width: 180 }}>
                  {MEMBERS.map(m => (
                    <button key={m} onClick={() => insertMention(m)} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 8px', background: 'none', border: 'none', color: '#ccc', fontSize: 14, cursor: 'pointer', borderRadius: 3, fontFamily: 'inherit' }} onMouseEnter={e => (e.currentTarget.style.background = '#2d2d2d')} onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
                      <Av name={m} size={18} />{m}
                    </button>
                  ))}
                </div>}
              ...
              <div key={c.id} style={{ display: 'flex', gap: 8, padding: '9px 0', borderBottom: '1px solid #2d2d2d' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ color: '#d4d4d4', fontSize: 14, fontWeight: 600 }}>{c.author}</span>
                    <span style={{ color: '#555', fontSize: 12 }}>{new Date(c.ts).toLocaleString()}</span>
                  ...
                  <p style={{ color: '#aaa', fontSize: 14, margin: '3px 0 0', lineHeight: 1.5 }}>{renderComment(c.text)}</p>
                </div>
              </div>
            ))}
            {(task.comments || []).length === 0 && <p style={{ color: '#444', fontSize: 14, textAlign: 'center', padding: 20 }}>No comments yet.</p>}
          </div>}

          {tab === 'activity' && <div style={{ color: '#888', fontSize: 14 }}>
            {[
